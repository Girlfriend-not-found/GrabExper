...
def simulationLogin():
    ...
    try:
        ...
        try:
            return sessions.post(url=url, headers=headers, json=data), sessions
        except RuntimeError:
...
def login():
    ...
    if loginText[0].text.find('登陆成功') != -1:
        log('模拟登录成功')
        return loginText[1]
    elif loginText[0].text.find('用户名或密码错误') != -1:
        log('登陆失败，用户名或密码错误，请检查后重新登录！')
        exit(1)
    elif loginText[0].text.find('验证码过期') != -1:
        log('验证码识别错误，请重新登录')
        exit(1)
    else:
